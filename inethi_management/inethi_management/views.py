...
def purchase(request, format=None):
    ...
    if request.method == 'POST':
        try:
            ...
            # Connect to readiusdesk database
            connection = mysql.connector.connect(host='rdmariadb',
                                        database='rd',
                                        user='rd',
                                        password='rd')
            radius_voucher = None
            ...
            if last_payment is not None:
                ...
                if delta.seconds > limit.payment_limit_period_sec or total_spent <= limit.payment_limit:
                    try:
                        if voucher:
                            ...
                            if connection.is_connected():
                                ...
                                mysqlQ = "select name from vouchers where status='new'"
                                mycursor.execute(mysqlQ)
                                voucher_list = mycursor.fetchall()
                                if len(voucher_list) > 0:
                                    radius_voucher=(voucher_list[0][0])
                            
                            # Finishing fetching a new voucher
                            
                            if radius_voucher is not None:
                                payment = Payment.objects.create(user_id=user, payment_method=payment_method, amount=amount,
                                                                ...
                                                                package=package,
                                                                voucher=radius_voucher)
                            else:
                                 return JsonResponse(status=400, data={'error': 'Cant obtain internet voucher from RadiusDesk'})
                        else:
                            payment = Payment.objects.create(user_id=user, payment_method=payment_method, amount=amount,
                                                             paydate_time=datetime.now(tz=pytz.UTC),
                                                             service_type_id=service_type,
                    ...
                    # Write to radiusdesk database change status to issued
